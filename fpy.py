...
class FpyHourly(FpyTemplate):
    ...
    def main(self, factory_name, shift_period):
        ...
        this_hour_file = this_hour_file_list[0]
        last_hour_file_list = self.get_last_hour_data(folder_path, shift_period)
        last_hour_file = last_hour_file_list[0]
        ...
        elif this_hour_file!=[] and last_hour_file==[] and this_hour_file[0]==today_file[0]:
...
if __name__=="__main__":
    fpy_hourly = FpyHourly()
    fpy_daily = FpyDaily()
    hour_range = 8
    for factory_name in ['CDE', 'FLEX', 'TwoWing']:
        for shift_hour in reversed(range(hour_range)):
            try:
                fpy_hourly.main(factory_name=factory_name, shift_period=shift_hour)
            except:
                print('Exception')
            try:
                fpy_daily.main(factory_name=factory_name, shift_period=shift_hour)
            except:
                print('Exception')
