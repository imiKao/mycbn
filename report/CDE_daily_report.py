...
class CDEDailyReportCpk(DailyReportTemplate):
    ...
    def main(self, factory_name, which_day, dashboard_id_info):
        ...
        weekno = (datetime.now() - timedelta(days=which_day)).weekday()
        which_day = which_day if weekno<6 else which_day+2
        query_text = "select MODEL, STATION_TYPE, TEST_ITEM, CPK from PREPROCESS_CPK_DAILY where CPK<1 and MODEL != 'unknow' and time >= now() - %sd and time < now() - %sd"%(which_day+1, which_day)
...
if __name__=="__main__":
    ...
    factory_list = ["CDE"]
    dashboard_id_dict = {
            "page_order_list": ["summary_page1", "summary_page2"],
            "summary": {"summary_page1": "NFU-UdnGz",
                        "summary_page2": "xcV9I0cMz"},
