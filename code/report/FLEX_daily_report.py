...
class FlexDailyReport(DailyReportTemplate):
    def __init__(self):
        super(FlexDailyReport, self).__init__()
    ...
    def main(self, factory_name, which_day, dashboard_id_info):
        ...
        print("which_day = ", which_day)
        query_text = "select MODEL, STATION_TYPE, TEST_ITEM, CPK from CPK_STATIONTYPE_DAILY where CPK<1 and time >= now() - %sd and time < now() - %sd"%(which_day+1, which_day)
        ...
        self.save_file_name = '%s_FLEX_daily_report'%(yesterday)

        """Add dashboard page"""
        pdf_list = []
        self.display_current_time("===== Step1: Summary dashboard =====")
        summary_pdf_list = self.add_group_dashboard_id([], dashboard_id_info, "summary", "summary_page1", which_day, "d")
        summary_pdf_list = self.add_group_dashboard_id(summary_pdf_list, dashboard_id_info, "summary", "summary_page2", which_day, "d")
...
if __name__=="__main__":
    ...
    factory_list = ["FLEX"]
    dashboard_id_dict = {
            "summary": {"summary_page1": "bGuEn5vMk",
                        "summary_page2": "4ZBkiz_Mz"},
            "cpk_analysis": {"test_item": "kV7pnkYGk",
                             "test_item_type": "h9SrsVYGz"}
    }
    try:
        obj.main(factory_name=factory_list[0],
