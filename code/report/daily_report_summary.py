...
class DailyReportSummary(DailyReportTemplate):
    ...
    def main(self, factory_name, grafana_url_id, which_day=1, cut_width=1366, cut_height=420):
        self.which_day = 3 if (datetime.now()-timedelta(days=which_day)).weekday()==6 else 1
    ...
    def get_min_cpk_from_influxdb(self, factory_name):
        ...
        query_text = "select min(CPK), MODEL, STATION_TYPE, TEST_ITEM from CPK_STATIONTYPE_DAILY where time >= now() - {}d and time < now() - {}d".format(self.which_day+1, self.which_day)
